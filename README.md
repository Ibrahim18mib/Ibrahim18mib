# Hi, I'm Mohammed Ibrahim 👋
**Full Stack Developer & DevOps Practitioner**

---

### 💻 About Me
I'm a **Full Stack Developer** with 5+ years of experience building **web, cloud, and desktop applications**.  
Proficient in **React.js, Angular, Electron.js, Node.js, NestJS, ASP.NET Core**, and experienced with **Oracle, SQL Server, and MongoDB**.  
Currently expanding my expertise in **Azure DevOps, CI/CD, Docker, and cloud automation**, integrating **DevOps practices** into my development workflow.  
Passionate about **turning complex ideas into seamless digital experiences**, from **UI/UX in Figma** to scalable backend systems.

---

### 🛠️ Tech Stack
- **Frontend:** React.js, Angular, Electron.js  
- **Backend:** Node.js, NestJS, ASP.NET Core  
- **Databases:** Oracle, SQL Server, MongoDB  
- **DevOps / Cloud:** Azure DevOps, CI/CD, Jenkins, Docker  
- **UI/UX Design:** Figma  

---

### 🚀 Projects
- [Flight-Booking-App](https://github.com/Ibrahim18mib/flight-book-ang17) – Angular WebApp
- [Quiz-Form](https://github.com/Ibrahim18mib/AngQuizForm) – Angular WebApp
- [Dynamic-Form-Builder](https://github.com/Ibrahim18mib/Ang-Dynamic-FormBuilder) – Angular WebApp
- [Fitness-Tracker](https://github.com/Ibrahim18mib/ANG-fitness-logger) – Angular WebApp
- [Doctor-Appointment](https://github.com/Ibrahim18mib/DoctorAppointment-Ang19) – Angular WebApp
- [Doctor-Appointment-API](https://github.com/Ibrahim18mib/DoctorAppointment-WebAPI) – Dotnet Core Web Api
- [Threads-App](https://github.com/Ibrahim18mib/threads-be) – Angular & NestJS app 
- [Ecommerce-App](https://github.com/Ibrahim18mib/mean-ecommerce) – Fullstack/MEAN
- [Todo-Listing-App](https://github.com/Ibrahim18mib/MERN-TodoLists) – Fullstack/MERN
- [Employee-CRUD](https://github.com/Ibrahim18mib/EmployeeCRUD-Asp.NET) – MVC/Asp.Net with Razor
- [Gadgets-Web-Api](https://github.com/Ibrahim18mib/GadgetWebApi-DotNet) – Web-Api/DotnetCore
- [SpeedMoto-Webapp](https://github.com/Ibrahim18mib/SpeedMoto_DotNetCore_MVC) – MVC/DotnetCore
- [Finance-Webapp](https://github.com/Ibrahim18mib/MyFinanceApp_CORE-MVC) – MVC/DotnetCore
- [Ng-Tar-Script](https://github.com/Ibrahim18mib/ng-tarbuild) – Script/Angular-transforming into tar file 
- [Calcelectron-Desktop App with Electron.js](https://github.com/Ibrahim18mib/Calcelectron) – Cross-platform desktop application  

---

### 📫 Connect with Me
- LinkedIn: [linkedin.com/in/Mohammed-Ibrahim](https://www.linkedin.com/in/mohammed-ibrahim-095660196/)  
- GitHub: [github.com/Ibrahim18mib](https://github.com/Ibrahim18mib)  
- Email: ibrahimmca18@gmail.com  

---

### 📊 GitHub Stats
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Ibrahim18mib&layout=compact)  
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Ibrahim18mib&show_icons=true&theme=radical)  

---

### ⚡ Fun Fact
- I enjoy **learning new DevOps tools and cloud technologies** while building scalable full-stack applications.  
- Always exploring ways to **automate workflows and improve development efficiency**.
